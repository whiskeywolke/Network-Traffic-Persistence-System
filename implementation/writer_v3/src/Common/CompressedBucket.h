...
namespace common {
    ...
    struct Entry {
        ...
        Entry(uint32_t v4Src, bool isSrc, int32_t timestampOffset, uint16_t portSrc, uint16_t portDst,
              uint8_t aProtocol,
              uint16_t length)
        ...
        bool add(const IPTuple &t) {
            ...
            if (!hasFirst) {
                ...
                if (!hasSecond) {
                    ...
                    if (firstEntry.v4Src == t.getV4Src() || firstEntry.v4Src ==
                                                            t.getV4Dst()) { //this means that the src of the first element address is equal in all others
                        matchedBySrc = true;
                    } else {
                        matchedBySrc = false;
                    }
                ...
                if (matchedBySrc) { //if we match by src we need to compare it to the src addrIndex since src of the first object is always equal
                    if (t.getV4Src() ==
                        ...
                        saveSrcAddr = false;
                               ...
                               firstEntry.v4Src) { //dst of the new object is equal to dst therefore save src
                        saveSrcAddr = true;
                    } else {
                    ...
                    if (t.getV4Src() ==
                        firstEntry.v4Dst) { //the src of the new object is equal to src therefore save dst
                        saveSrcAddr = false;
                    } else if (t.getV4Dst() ==
                               firstEntry.v4Dst) { //dst of the new object is equal to dst therefore save src
                        saveSrcAddr = true;
                    } else {
                        std::cout << "nothing equal" << std::endl;
                        assert(false);
...
//////////////
               ...
               */
                ...
                int32_t timestampOffset = (t.getTvSec() * 1000000 + t.getTvUsec()) -
                                          firstEntry.timestamp;
                /*         if(firstEntry.timestamp<=(t.getTvSec() * 1000000 + t.getTvUsec())){
                             timestampOffset = 0; //in case the packets arrive out of order the offset will be set to 0
